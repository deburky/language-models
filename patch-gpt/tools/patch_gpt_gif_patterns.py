...
DURATION = 0.1  # 0.15  # Duration of each frame in the GIF (in seconds)
...
class PatchGPT(nn.Module):
    ...
    def __init__(self, input_dim, seq_len, dim, heads, depth):
        ...
        self.final_norm = nn.LayerNorm(dim)  # Add normalization layer
...
def generate_image(model, patch_size, img_size, seq_len, input_dim, device, epoch, data_path):
    ...
    for step in range(seq_len - 1):
        ...
        ax.axis("off")
        ax.set_title(f"Patch {step + 1} (context={seq_len})", color="black")
    ...
    Path("../images/outputs_gif_patterns").mkdir(parents=True, exist_ok=True)
    gif_path = f"../images/outputs_gif_patterns/gen_{epoch:03d}.gif"
...
@app.command()
def train(
    data_path: str = typer.Option("images/patterns", help="Folder with training images"),
    num_epochs: int = typer.Option(2000, help="Total training epochs"),
    ...
    patch_size: int = typer.Option(4, help="Patch size"),
    img_size: int = typer.Option(32, help="Input image resolution"),
